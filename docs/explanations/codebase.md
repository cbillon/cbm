...
Dans notre cas cela comprend les sources de Moodle , ainsi que les sources des plugins.
Les sources sont disponibles sous format de fichier zip ou depuis un depot git.
Les sources sont versionnés grâce à git système de gestion de versions le plus corramment utilisé.
...
La version de plugins à installer dépend de la version de Moodle
...
Le script permet de gérer plusieurs instances de base de code d'une même version Moodle ou d'une version différentes avec pour chaque instance sa propre liste de plugins.
...
Le déploiement doit prendre en compte tout ce qui est susceptible de varier entre des déploiements (validation, production, environnement de développement, etc.). 
dans le cas de Moodle cela est défini par le fichier configuration config.php
Déploiement
- base de code du projet + config.php correspondant à l'environnement
