...
class api {
     ...
     * @return \core\update\remote_info|bool
     */
    public function find_plugin($component, $reqversion=ANY_VERSION, $branch) {
    ...
    protected function call_service($serviceurl, array $params=array()) {
        ...
        if (!empty($curlerrno)) {
            echo 'cURL: Error '.$curlerrno.' when calling '.$serviceurl."\n";
            return false;
        }
        ...
        if (isset($response->info['ssl_verify_result']) and $response->info['ssl_verify_result'] != 0) {
            echo 'cURL/SSL: Unable to verify remote service response when calling '.$serviceurl."\n";
            return false;
        }
        */
