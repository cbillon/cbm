...
class curl {
    ...
    public function __construct($settings = array()) {
        ...
        if (!empty($settings['cookie'])) {
            if($settings['cookie'] === true) {
                $this->cookie = $_CFG->dataroot.'/curl_cookie.txt';
            } else {
                $this->cookie = $settings['cookie'];
            }
        }
        if (!empty($_CFG->proxyhost)) {
            if (empty($_CFG->proxyport)) {
                $this->proxy_host = $_CFG->proxyhost;
            } else {
                $this->proxy_host = $_CFG->proxyhost.':'.$_CFG->proxyport;
            }
            if (!empty($_CFG->proxyuser) and !empty($_CFG->proxypassword)) {
                $this->proxy_auth = $_CFG->proxyuser.':'.$_CFG->proxypassword;
                $this->setopt(array(
                            'proxyauth'=> CURLAUTH_BASIC | CURLAUTH_NTLM,
                            'proxyuserpwd'=>$this->proxy_auth));
            }
            if (!empty($_CFG->proxytype)) {
                if ($_CFG->proxytype == 'SOCKS5') {
    ...
    public static function get_cacert() {
        global $TOOLPATH, $_CFG;

        // Windows PHP does not have any certs, we need to use something.
        if ($_CFG->ostype === 'WINDOWS') {
            if (is_readable($TOOLPATH."/certs/cacert.pem")) {
                return realpath($TOOLPATH."/certs/cacert.pem");
            }
        }

        // Bundle in dataroot always wins.
        if (is_readable($TOOLPATH."/certs/moodleorgca.crt")) {
            return realpath($TOOLPATH."/certs/moodleorgca.crt");
        }
    ...
    public function setopt($options = array()) {
        if (is_array($options)) {
            foreach ($options as $name => $val) {
                if (!is_string($name)) {
                    throw new Exception('Curl options should be defined using strings, not constant values.');
                }
    ...
    public function download_one($url, $params, $options = array()) {
        ...
        if (!empty($options['filepath']) && empty($options['file'])) {
            // open file
            if (!($options['file'] = fopen($options['filepath'], 'wb'))) {
                $this->errno = 100;
                return "Error in writing zip file";
            }
